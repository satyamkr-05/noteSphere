...
export default function ExplorePage({ onNotesChanged, showToast }) {
  ...
  useReveal([notes.length, isLoading, courses.length]);
  ...
  async function loadCourses() {
    try {
      ...
      setCourses([]);
      showToast(getErrorMessage(error, "Unable to load courses right now."), "error");
    }
  }
      ...
      <div className="container">
        ...
        <div className="explore-toolbar glass-card reveal">
          <div className="search-field">
            ...
            <input
              type="text"
              placeholder="Search by title, subject, topic, branch, or course"
        ...
        {courses.length > 0 ? (
          <div className="course-shortcuts glass-card reveal is-visible">
            <span className="course-shortcuts__label">Courses</span>
            <div className="course-shortcuts__list">
              {courses.map((course) => (
                <button
                  key={course}
                  type="button"
                  className={`course-shortcuts__button${selectedCourse === course ? " is-active" : ""}`}
                  onClick={() => {
                    setSelectedCourse((current) => (current === course ? "" : course));
                    setCurrentPage(1);
                  }}
                >
                  {course}
            ...
            <div className="notes-grid notes-grid--subject">
              {subjectNotes.map((note) => (
                ...
                >
                  ...
                  <h3>{note.title}</h3>
                  <p>{note.topicName || note.description}</p>
